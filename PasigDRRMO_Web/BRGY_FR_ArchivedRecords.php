...
<head>
    ...
    <title> PCDRRMO | BRGY Archived Responders </title>
    <link rel="stylesheet" href="BRGY_FR_ArchivedRecords45.css">
...
<div class="bottom_content">
 ...
 <section class="home-section">
    <h1> FIRE RESPONDERS ARCHIVE ACCOUNT </h1>
...
if (isset($_GET['restore_id'])) {
    ...
    $restore_sql = "INSERT INTO firerespondersaccount (ID, Barangay,Profile, Name, EmailAddress, Password, ContactNumber, Birthday, Address) 
                    SELECT ID, Barangay, Profile, Name, EmailAddress, Password, ContactNumber, Birthday, Address 
                    FROM firerespondersarchive WHERE ID = ?";
    $stmt_restore = $conn->prepare($restore_sql);
    $stmt_restore->bind_param("i", $restore_id);

    $action = "Restored a fire responder account";
    $timestamp = date("Y-m-d H:i:s");
    $stmt_report = $conn->prepare("INSERT INTO reports (ACCOUNT_NAME, ROLE, ACTION, TIMESTAMP) VALUES (?, ?, ?, ?)");
    $stmt_report->bind_param("ssss", $user, $position, $action, $timestamp);

    if ($stmt_restore->execute() && $stmt_report->execute()) {
        $delete_sql = "DELETE FROM firerespondersarchive WHERE ID = ?";
        $stmt_delete = $conn->prepare($delete_sql);
        $stmt_delete->bind_param("i", $restore_id);
...
$sql = "SELECT ID, Barangay, Profile, Name, EmailAddress, Password, ContactNumber, Birthday, Address FROM firerespondersarchive";
$result = $conn->query($sql);
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                echo "<td style='display: none;'>".$row["Barangay"]."</td>";
                echo "<td><img src='" . htmlspecialchars($row["Profile"]) . "' alt='Profile Picture' class='profile-pic' style='width: 50px; height: 50px; border-radius: 50%; object-fit: cover;'></td>";
                echo "<td>".$row["Name"]."</td>";
                ...
                echo "<td> 
                    ...
                    <button class='view-button'><a href='BRGY_FR_ArchivedRecords.php?restore_id=" . $row["ID"] . "' class='restore' type='submit'> <img src='images/restore.png' alt='View Icon'> </button>
                    </div>
                 </td>";
                echo "</tr>";
            }
