...
<head>
    ...
    <title> PCDRRMO | C3 Announcements </title>
    <link rel="stylesheet" href="C3_SeeAnnouncements55.css">
...
if (isset($_GET['archive_id'])) {
    ...
    $archive_sql = "INSERT INTO c3_archive SELECT * FROM c3_announcement WHERE ID = ?";
    $archive_stmt = $conn->prepare($archive_sql);
    $archive_stmt->bind_param("i", $archive_id); // Bind the archive_id
    ...
    if ($archive_stmt->execute()) {
        $delete_sql = "DELETE FROM c3_announcement WHERE ID = ?";
        $delete_stmt = $conn->prepare($delete_sql);
        $delete_stmt->bind_param("i", $archive_id);
        if ($delete_stmt->execute()) {
            echo "<script>alert('Announcement archived successfully!');</script>";
        } else {
...
$sql = "SELECT ID, Date, Subject, Message, Photo FROM c3_announcement";
if (!empty($searchQuery)) {
    $sql .= " WHERE Subject LIKE ? OR Message LIKE ?";
} 
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<p class='announcement-date'>Date: " . $row["Date"] . "</p> <a href='C3_SeeAnnouncement.php?archive_id=" . $row["ID"] . "' class='archive' type='submit'>Archive</a> <br>";
        echo '</div>';
