...
<head>
    ...
    <title> PCDRRMO | C3 Archived Announcements </title>
    <link rel="stylesheet" href="C3_TrashbinAnnouncements55.css">
...
<ul class="menu_items">
    ...
    <!-- ANNOUNCEMENT -->
    <li class="item">
        <a href="C3_Announcements.php" class="nav_link active">
            <span class="navlink_icon">
...
if (isset($_SESSION["Username"]) && isset($_SESSION["Position"])) {
    ...
    $action = "Viewed archived announcements";
...
<div class="home-container1">
    <h2 class="announcement-heading">
        <img src="images/Announcement.png" alt="Announcement" class="announcement-image">
        Archived Announcements
    </h2>
    <div class="x-account">
    <a href="C3_Announcements.php" class="x-link">X</a>
    </div>
</div>
<hr>
...
if (isset($_GET['restore_id'])) {
    ...
    $restore_sql = "INSERT INTO c3_announcement SELECT * FROM c3_archive WHERE ID = $restore_id";

    $timestamp = date("Y-m-d H:i:s");
    $action = "Restored a record";
    $stmt2 = $conn->prepare("INSERT INTO reports (ACCOUNT_NAME, ROLE, ACTION, TIMESTAMP) VALUES (?, ?, ?, ?)");
    $stmt2->bind_param("ssss", $username, $position, $action, $timestamp);
    ...
    if ($conn->query($restore_sql) === TRUE) {
        $delete_sql = "DELETE FROM c3_archive WHERE ID = $restore_id";
        if ($conn->query($delete_sql) === TRUE) {
            echo "<script>alert('Announcement restored successfully!');</script>";
        } else {
...
$sql = "SELECT ID, Date, Subject, Message, Photo FROM c3_archive ORDER BY ID DESC";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    echo '<div class="announcement-container-wrapper">';
    while ($row = $result->fetch_assoc()) {
        echo '<div class="announcement-container">';
        echo "<div class='announcement-header'>";
        echo "<p class='announcement-date'>Date: " . $row["Date"] . "</p> <a href='C3_TrashbinAnnouncement.php?restore_id=" . $row["ID"] . "' class='archive' type='submit'>Restore</a> <br>";
        echo '</div>';
