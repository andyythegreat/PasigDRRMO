...
<head>
    ...
    <title> PCDRRMO | C3 Resident Accounts </title>
    <link rel="stylesheet" href="C3_ResidentAccount35.css">
...
$total_sql = "SELECT COUNT(*) as total FROM pasigresident";
$total_result = $conn->query($total_sql);
$total_row = $total_result->fetch_assoc();
$total_records = $total_row['total'];
$total_pages = ceil($total_records / $limit);

$sql = "SELECT ID, Barangay, Profile, Email, Fullname, Username, ContactNumber, Birthday 
        FROM pasigresident 
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        while ($row = $result->fetch_assoc()) {
                            echo "<tr>
                                    ...
                                    <td>{$row['Barangay']}</td>
                                    <td><img src='{$row['Profile']}' alt='Profile' style='width: 50px; height: 50px; border-radius: 50%; object-fit: cover;'></td>
                                    <td>{$row['Email']}</td>
                                    <td>{$row['Fullname']}</td>
                                    <td>{$row['Username']}</td>
        ...
        <div class="button-container">
            <div class="prev-next-buttons">
                ...
                <?php if ($current_page > 1): ?>
                    <a href="?page=<?php echo $current_page - 1; ?>" class="prev-next-button"> < Prev </a>
                <?php endif; ?>

                <!-- Next button -->
                <?php if ($current_page < $total_pages): ?>
                    <a href="?page=<?php echo $current_page + 1; ?>" class="prev-next-button">Next > </a>
                <?php endif; ?>
            </div>
        </div>
    </div>
