...
<head>
    ...
    <title> PCDRRMO | C3 Declined Reports </title>
    <link rel="stylesheet" href="C3_MDeclinedReports55.css">
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['decline'])) {
    ...
    $sql_insert = "INSERT INTO mobilelocate_declined (Date, Caller, Location, Involve) VALUES (?, ?, ?, ?)";
    $stmt = $conn->prepare($sql_insert);
    $stmt->bind_param("ssss", $row['Date'], $row['Caller'], $row['Location'], $row['Involve']);
    ...
    if ($stmt->affected_rows > 0) {
        ...
        echo "Error declining record: " . $conn->error;
    }
}

$sql_select_declined = "SELECT * FROM mobilelocate_declined ORDER BY ID DESC";
$result_declined = $conn->query($sql_select_declined);
...
?>
 ...
 <section class="home-section">
    <h1>DECLINED MOBILE REPORTS </h1>

    <div class="home-container1">
    <h2>INFORMATION</h2> 
    <div class="x-account">
        <a href="C3_MReports.php" class="x-link">X</a>
    </div>
    </div> 
    ...
    if ($result_declined->num_rows > 0) {
        while ($row_declined = $result_declined->fetch_assoc()) {
            ...
            echo "<p>Date: " . $row_declined["Date"] . "</p>";
            echo "<p>Caller: " . $row_declined["Caller"] . "</p>";
            echo "<p>Location: " . $row_declined["Location"] . "</p>";
            echo "<p>Involved: " . $row_declined["Involve"] . "</p>";
            echo "<p style='color: #333; font-weight: bold; display: inline;'>Status: <span style='color: red; font-weight: bold; background-color: #e0e0e0; padding: 5px 10px; border-radius: 5px;'>Declined</span></p>";
            echo "</div>";
            echo "</div>";
            echo "</div>";
        }
