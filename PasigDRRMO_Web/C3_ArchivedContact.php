...
$sql = "SELECT ID, Contact_name, Contact_number FROM c3_archivecontactinfo";
...
<head>
    ...
    <title> PCDRRMO | C3 Archived Hotlines </title>
    <link rel="stylesheet" href="C3_ArchivedContact55.css">
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['restore_id'])) {
    ...
    try {
        // Fetch the contact details from the archive
        $sql = "SELECT Contact_name, Contact_number FROM c3_archivecontactinfo WHERE ID = ?";
        ...
        if ($contact) {
            // Insert the contact into c3_contactinfo
            $sql = "INSERT INTO c3_contactinfo (Contact_name, Contact_number) VALUES (?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("ss", $contact['Contact_name'], $contact['Contact_number']);
            $stmt->execute();
            $stmt->close();

            // Delete the contact from c3_archivecontactinfo
            $sql = "DELETE FROM c3_archivecontactinfo WHERE ID = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("i", $restore_id);
            $stmt->execute();
            $stmt->close();
            ...
            $conn->commit();
                echo "<script>alert('Contact restored successfully'); window.location.href='C3_ContactInfo.php';</script>";
            exit();
        } else {
            $conn->rollback();
            echo "Failed to restore contact.";
        }
    } catch (Exception $e) {
        $conn->rollback();
...
<section class="home-section">
    <h1> ARCHIVED CONTACT INFORMATION </h1>
    
    <div class="home-container1">
    <div class="x-account">
        <a href="C3_ContactInfo.php" class="x-link">X</a>
    </div>
    </div> 
            ...
            <tbody>
                <?php if ($result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                            <td class='hidden-column'>" . htmlspecialchars($row['id']) . "</td>
...
<td>
    <div class='btn-container'>
        <form method='post' action='C3_ArchivedContact.php'>
            <input type='hidden' name='restore_id' value='<?php echo htmlspecialchars($row['ID']); ?>'>
            <button class='update-button' type='submit'>
                <img src='images/restore.png' alt='Restore Icon'>
            </button>
        </form>
    </div>
</td>
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="3">No archived contact information available.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
