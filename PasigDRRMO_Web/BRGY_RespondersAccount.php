...
<head>
    ...
    <title>Pasig DRRMO | Add Responder Account </title>
    <link rel="stylesheet" href="BRGY_RespondersAccount55.css">
...
<nav class="navbar">
    <div class="logo_item">
        <img src="<?php echo htmlspecialchars($generalLogoPath); ?>" alt=""></i>Pasig DRRMO
    </div>
...
<ul class="menu_items">
    ...
    <!-- FIRE RESPONDERS -->
    <li class="item">
        <a href="BRGY_FResponders.php" class="nav_link active">
            <span class="navlink_icon">
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($fileExtension, $allowedExtensions)) {
            ...
            $dest_path = $uploadFileDir . basename($fileName); // Ensure unique names to avoid overwrite
    ...
    $stmt = $conn->prepare("INSERT INTO firerespondersaccount (Barangay, Name, Username, EmailAddress, Password, ContactNumber, Birthday, Address, Profile) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");

    // Bind parameters
    $stmt->bind_param("sssssssss", $barangay, $name, $username, $email, $password, $contact_number, $birthday, $address, $photoPath);

    if ($stmt->execute()) {
        // Log the action in the reports table
        $action = "Added a Fire Responder account";
        $timestamp = date('Y-m-d H:i:s'); // Set the timestamp
        $stmt2 = $conn->prepare("INSERT INTO reports (ACCOUNT_NAME, ACTION, TIMESTAMP) VALUES (?, ?, ?)");
        $stmt2->bind_param("sss", $username, $action, $timestamp);
        $stmt2->execute();
...
<section class="home-section">
    ...
    <div class="x-container">
        <div class="x-account">
            <a href="BRGY_FResponders.php" class="x-link"> X </a>
        </div>
    </div>

    <div class="home-container">  
        <form action="BRGY_RespondersAccount.php" method="post" enctype="multipart/form-data">

            <label for="barangay">Barangay:</label>
            <input type="text" id="barangay" name="Barangay" value="<?php echo $user; ?>" required> <br> <br>

            <label for="updatePhotos">Profile:</label>
            <input type="file" id="updatePhotos" name="photo">  

            <div id="update-photo-preview-container" style="width: 120px; height: 120px; border-radius: 50%; background-color: #0056b3; display: flex; align-items: center; justify-content: center; margin-top: 20px; color: white; font-size: 170px; text-align: center;">
                <img id="update-photos-preview" style="width: 100%; height: 100%; border-radius: 50%; object-fit: cover; display: none;" alt="Photo Preview">  
                <span id="update-photos-default-text" style="font-size: 100px;">+</span> 
            </div> <br> 
            ...
            <input type="text" id="contact_number" name="ContactNumber" required> <br> <br>
