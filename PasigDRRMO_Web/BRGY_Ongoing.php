...
$sql = "SELECT ID, Date, Caller, Location, Involve, Status 
        FROM c3_locate 
        WHERE Status != 'Resolved' AND DATE(Date) = CURDATE()
        ORDER BY ID DESC";
...
<head>
    ...
    <link rel="stylesheet" href="BRGY_Ongoing80.css">
...
function fetchNotifications() {
    ...
    xhr.onload = function () {
        if (xhr.status === 200) {
            ...
            // Display all notifications (read and new)r
...
$sql = "SELECT ID, Date, Caller, Location, Involve, Status 
        ...
        AND Status != 'Resolved'";  // Exclude 'Resolved' status

if (!empty($callerFilter) && $callerFilter !== "Show All") {
    $sql .= " AND Caller = '$callerFilter'";
}

$sql .= " ORDER BY resolved_time DESC";  // You can still order by resolved_time if needed
...
$result = $conn->query($sql);
        ...
        echo "<a href='BRGY_Request.php?id=" . urlencode($row['ID']) . "' class='request-button'>Request</a>";
        echo "</div>";
        echo "</div>";
