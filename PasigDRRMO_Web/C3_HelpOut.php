...
$sql = "SELECT ID, Date, Location, Involve, TypeOfTruck 
        FROM brgy_helpout 
...
<head>
    ...
    <title> PCDRRMO | C3 Help Out </title>
    <link rel="stylesheet" href="C3_HelpOut.css">
...
<ul class="menu_items">
    ...
    <ul class="menu_items">
        ...
        <!-- HELPOUT -->
        <li class="item">
            <a href="C3_HelpOut.php" class="nav_link active">
                <span class="navlink_icon">
...
<section class="home-section">
    <h1> HELP OUT - OUTSIDE PASIG </h1>

    <div class="home-container">
    <h2> INFORMATION </h2> <br>
    
    <div class="search-bar">
    <form action="C3_HelpOut.php" method="GET">
    ...
    <?php
        // Make sure the 'search' parameter is set
        $search = isset($_GET['search']) ? $_GET['search'] : '';

        // SQL query with filter if search is provided
        $sql = "SELECT ID, Date, Location, Involve, TypeOfTruck FROM brgy_helpout WHERE 1=1";
        
        if (!empty($search)) {
            // Add search filter if search term is not empty
            $sql .= " AND (Location LIKE '%$search%' OR Involve LIKE '%$search%' OR TypeOfTruck LIKE '%$search%')";
        }
        
        // Optionally, filter by today's date (as in your previous question)
        date_default_timezone_set('Asia/Manila');
        $today = date('Y-m-d');
        $sql .= " AND DATE(Date) = '$today'"; // Filter by today's date in PHT
...
<script>
    function clearSearch() {
        ...
        window.location.href = 'C3_HelpOut.php'; // Reload the page to clear the search results
    }
</script>
