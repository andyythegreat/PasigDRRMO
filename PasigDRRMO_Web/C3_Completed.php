...
<head>
    ...
    <title> PCDRRMO | C3 Completed </title>
    <link rel="stylesheet" href="C3_Completed55.css">
...
<ul class="menu_items">
    ...
    <ul class="menu_items">
        ...
        <!-- COMPLETED -->
        <li class="item">
            <a href="C3_Completed.php" class="nav_link active">
                <span class="navlink_icon">
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id'])) {
    ...
    $action = "Resolved a fire incident";
    $stmt = $conn->prepare("INSERT INTO reports (ACCOUNT_NAME, ROLE, ACTION, TIMESTAMP) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $user, $position, $action, $timestamp);

    $success = $stmt->execute();

    $updateSql = "UPDATE c3_locate SET Status = 'Resolved' WHERE ID = ?";
...
if (isset($_GET['search']) && !empty($_GET['search'])) {
    $search = $_GET['search'];
    $search_query = " AND (Date LIKE '%$search%' OR Caller LIKE '%$search%' OR Location LIKE '%$search%' OR Involve LIKE '%$search%' OR Status LIKE '%$search%')";
}

$sql = "SELECT c3_locate.ID, c3_locate.Date, c3_locate.Caller, c3_locate.Location, c3_locate.Involve, c3_locate.Status, 
               mr.Username as Responders, bp.Barangay, bp.UnitName
        FROM c3_locate
        LEFT JOIN mobile_respond mr ON c3_locate.ID = mr.OngoingID
        LEFT JOIN brgy_profile bp ON mr.TruckID = bp.ID
        WHERE c3_locate.Status = 'Resolved' 
        AND (
            c3_locate.Date LIKE '%$search%' OR 
            c3_locate.Caller LIKE '%$search%' OR 
            c3_locate.Location LIKE '%$search%' OR 
            c3_locate.Involve LIKE '%$search%' OR 
            c3_locate.Status LIKE '%$search%' OR 
            mr.Username LIKE '%$search%' OR 
            bp.Barangay LIKE '%$search%' OR 
            bp.UnitName LIKE '%$search%'
        )
        ORDER BY c3_locate.resolved_time DESC";
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $truckDetails = isset($row["Barangay"]) && isset($row["UnitName"]) ? $row["Barangay"] . ": " . $row["UnitName"] : "No truck details";
        echo "<p>Responders: " . $truckDetails . "</p>";        
        echo "<p style='color: #333; font-weight: bold; display: inline;'>Status: <span style='color: #062B82; font-weight: bold; padding: 5px 10px; border-radius: 5px; background-color: #e0e0e0'>" . $row["Status"] . "</span>";
        echo "</div>";
        echo "</div>";
        echo "</div>";
    }
...
<script>
    function clearSearch() {
        ...
        window.location.href = 'C3_Completed.php'; 
    }
</script>
