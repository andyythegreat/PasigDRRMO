...
<head>
    ...
    <title>PCDRRMO | C3 Barangay Reports </title>
    <link rel="stylesheet" href="C3_BReport55.css">
...
<ul class="menu_items">
    ...
    <!-- BARANGAY REPORTS -->
    <li class="item">
        <a href="C3_BReports.php" class="nav_link active">
            <span class="navlink_icon">
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['accept'])) {
    ...
    $sql_select = "SELECT * FROM brgy_locate WHERE ID = ?";
    ...
    if ($stmt->affected_rows > 0) {
        ...
        $sql_insert_accept = "INSERT INTO brgy_accept (EventLog, Date, Caller, Location, Involve, Status) VALUES (?, ?, ?, ?, ?, ?)";
        $status = 'Accepted';
        $action_accept = "C3 has just approved your report!";
        $stmt_accept = $conn->prepare($sql_insert_accept);
        $stmt_accept->bind_param("ssssis", $action_accept, $row['Date'], $row['Caller'], $row['Location'], $row['Involve'], $status);
        $stmt_accept->execute();
        ...
        if ($success) {
            ...
            if (!empty($tokens)) {
                ...
                foreach ($tokens as $token) {
                    // Use the original $row for location
...
if (isset($_POST['decline'])) {
    ...
    $sql_insert_declined = "INSERT INTO c3_breports_declined (Date, Caller, Location, Involve) VALUES (?, ?, ?, ?)";
    $stmt_insert = $conn->prepare($sql_insert_declined);
    $stmt_insert->bind_param("ssss", $row['Date'], $row['Caller'], $row['Location'], $row['Involve']);

    if ($stmt_insert->execute()) {
        $sql_insert_decline = "INSERT INTO brgy_decline (EventLog, Date, Caller, Location, Involve, Status) VALUES (?, ?, ?, ?, ?, ?)";
        $status_decline = 'Declined';
        $action_decline = "C3 has just rejected your report!";
        $stmt_decline = $conn->prepare($sql_insert_decline);
        $stmt_decline->bind_param("ssssis", $action_decline, $row['Date'], $row['Caller'], $row['Location'], $row['Involve'], $status_decline);
        ...
        echo "Report declined successfully and moved to declined reports.";
    } else {
        echo "Error inserting declined report: " . $stmt_insert->error;
    }
}

$currentDate = date("Y-m-d");

$sql_select = "SELECT * FROM brgy_locate WHERE DATE(Date) = ? ORDER BY ID DESC";
$stmt = $conn->prepare($sql_select);
...
<head>
 <section class="home-section">
    ...
    <h2>INFORMATION</h2></head>
        <div class="add-account">
            <a href="C3_DeclinedReports.php" class="see"> Declined Reports </a>
        </div>
    </div>
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        // Unique initMap function for this entry
        echo "<script>
            var icon = {
...
document.querySelectorAll('.accept-button').forEach(button => {
    button.addEventListener('click', function() {
        ...
        xhttp.open("POST", "BRGY_RAlarm.php", true);
