...
if (isset($data['id'], $data['caller'], $data['location'], $data['barangay'], $data['involve'], $data['status'])) {
    ...
    if ($stmt->execute()) {
        ...
        $stmt_decline = $conn->prepare("INSERT INTO mobilelocate_declined (EventLog, Date, Caller, Location, Involve, Status) VALUES (?, ?, ?, ?, ?, ?)");
        ...
        if ($stmt_decline->execute()) {
            $deleteMobileStmt = $conn->prepare("DELETE FROM mobilelocate WHERE Caller = ? AND Location = ? AND Barangay = ? AND Involve = ? AND Status = ?");
            $deleteMobileStmt->bind_param("sssss", $data['caller'], $data['location'], $data['barangay'], $data['involve'], $data['status']);
            
            if ($deleteMobileStmt->execute()) {
                echo json_encode(['message' => 'Mobile alert declined successfully']);
            } else {
                echo json_encode(['message' => 'Failed to delete from mobile_locate.']);
            }

            $deleteMobileStmt->close();
        } else {
