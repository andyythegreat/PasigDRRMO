...
$sql = "SELECT ID, Date, Location, Barangay, Involve, Status 
        FROM brgy_helpout 
...
<head>
    ...
    <link rel="stylesheet" href="BRGY_FiledHelpOut25.css">
...
<ul class="menu_items">
    ...
    <!-- ONGOING -->
    <li class="item">
        <a href="BRGY_Ongoing.php" class="nav_link">
            <span class="navlink_icon">
...
<section class="home-section">
    ...
    <h2> INFORMATION </h2>
    <div class="x-account">
        <a href="BRGY_HelpOut.php" class="x-link">X</a>
    </div>
    </div> 
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['callerFilter'])) {
    ...
    $sql = "SELECT ID, Date, Location, Barangay, Involve, Status 
            FROM brgy_helpout 
            WHERE DATE(Date) = CURDATE()"; // Filter for today's data

    if (!empty($callerFilter) && $callerFilter !== "Show All") {
        $sql .= " AND Barangay = '$callerFilter'"; // Filter by Barangay
    }
    $sql .= " ORDER BY ID DESC"; // Sort by ID
    $result = $conn->query($sql);
}

if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $statusColor = '#90ee90'; // Default status color

        // Fetch color for the status dynamically
        ...
        echo "<div id='map-" . $row['ID'] . "' style='margin-top: -20px; height: 350px; width: 100%;'></div>";
        echo "</div>";
        echo "</div>";
        ...
        echo "<script>
            ...
            // Initialize this map on page load
