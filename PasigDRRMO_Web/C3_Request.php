...
<head>
    ...
    <title>Pasig DRRMO | Request </title>
    <link rel="stylesheet" href="C3_Request150.css">
...
$query = "SELECT Responder, Barangay, Request, BStatus FROM c3_request WHERE OngoingID = ?";
...
<script>
    // Function to fetch Resource Deployment Requests
    function fetchResourceRequests() {
        var id = <?php echo $id; ?>;
        
        var xhr = new XMLHttpRequest();
        xhr.open("GET", "fetch_requests.php?id=" + id, true);
        xhr.onload = function() {
            if (xhr.status == 200) {
    ...
    // Refresh the table every 5 seconds to update new requests
    setInterval(fetchResourceRequests, 5000);
