...
<head>
    ...
    <title> PCDRRMO | BRGY Completed </title>
    <link rel="stylesheet" href="BRGY_Completed75.css">
...
<ul class="menu_items">
    ...
    <!-- COMPLETED -->
    <li class="item">
        <a href="BRGY_Completed.php" class="nav_link active">
            <span class="navlink_icon">
...
// Handle form submission from C3_Ongoing.php to update status
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id'])) {
    $id = $_POST['id'];
    
    // Update status to 'Resolved'
    $updateSql = "UPDATE c3_locate SET Status = 'Resolved' WHERE ID = $id";
...
$sql = "SELECT ID, Date, Caller, Location, Involve, Status FROM c3_locate WHERE Status = 'Resolved' ORDER BY resolved_time DESC";
...
<section class="home-section">
    ...
    <label for="callerFilter">Filter by Caller: &nbsp; </label>
    <select id="callerFilter" name="callerFilter">
...
<?php
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['callerFilter'])) {
    $callerFilter = $_POST['callerFilter'];
    $sql = "SELECT ID, Date, Caller, Location, Involve, Status FROM c3_locate WHERE Status = 'resolved'"; // Add Status filter
    if (!empty($callerFilter) && $callerFilter !== "Show All") {
        $sql .= " AND Caller = '$callerFilter'"; // Add caller filter if selected
    }
    $sql .= " ORDER BY resolved_time DESC";
    $result = $conn->query($sql);
}
?>
...
if ($result->num_rows > 0) {
    // Output completed entries
    while ($row = $result->fetch_assoc()) {
        echo "<div class='home-container2'>";
        ...
        echo "<p style='color: #333; font-weight: bold; display: inline;'>Status: <span style='color: #062B82; font-weight: bold; padding: 3px 5px; border-radius: 5px; background-color: #e0e0e0'>" . $row["Status"] . "</span>";
        echo "</div>";
        echo "</div>";
        echo "</div>";
    }
