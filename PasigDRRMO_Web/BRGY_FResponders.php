...
<head>
    ...
    <title> PCDRRMO | BRGY Fire Responders </title>
    <link rel="stylesheet" href="BRGY_FResponders55.css">
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['archive_id'])) { 
        ...
        $archive_sql = "INSERT INTO firerespondersarchive (ID, Barangay, Profile, Name, EmailAddress, Password, ContactNumber, Birthday, Address) 
                        SELECT ID, Barangay, Profile, Name, EmailAddress, Password, ContactNumber, Birthday, Address 
                        FROM firerespondersaccount WHERE ID = ?";
        $stmt = $conn->prepare($archive_sql);
        $stmt->bind_param("i", $archive_id);

        $action = "Archived a fire responder account";
        ...
        if ($stmt->execute() && $stmt2->execute()) {
            $delete_sql = "DELETE FROM firerespondersaccount WHERE ID = ?";
...
$barangay_query = "SELECT Username FROM c3barangayaccount WHERE Username = ?";
$barangay_stmt = $conn->prepare($barangay_query);
$barangay_stmt->bind_param("s", $user);
$barangay_stmt->execute();
$barangay_result = $barangay_stmt->get_result();

if ($barangay_result->num_rows > 0) {
    $barangay_row = $barangay_result->fetch_assoc();
    $barangay = $barangay_row["Username"];
    
    $sql = "SELECT * FROM firerespondersaccount WHERE Barangay = ?";
    $stmt = $conn->prepare($sql);
...
?>
 ...
 <section class="home-section">
    <h1> OFFICERS IN CHARGE ACCOUNTS </h1>

    

    <div class="account-container">
    <div class="add-account">
        <a href="BRGY_FR_ArchivedRecords.php" class="add-account-link"> Archived
        </a>
    
        <a href="BRGY_RespondersAccount.php" class="add-account-link">
    ...
    <div class="table-container">
        <table>
            ...
            <tbody>
                <?php
                $barangay_query = "SELECT Username FROM c3barangayaccount  WHERE Username = '$user'";
                ...
                if ($barangay_result->num_rows > 0) {
                    ...
                    $sql = "SELECT * FROM firerespondersaccount WHERE Barangay = '$barangay'";
                    ...
                    if ($result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            ...
                            <div class='btn-container'>
                                <form action='BRGY_FResponders.php' method='POST'>
