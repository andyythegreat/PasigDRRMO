...
<head>
    ...
    <title> PCDRRMO | C3 Audit Trail </title>
    <link rel="stylesheet" href="C3_Reports25.css">
...
<ul class="menu_items">
    ...
    <!-- REPORTS -->
    <li class="item">
        <a href="C3_Reports.php" class="nav_link active">
            <span class="navlink_icon">
...
$sql = "SELECT ID, ACCOUNT_NAME, ROLE, ACTION, TIMESTAMP FROM reports WHERE (ID LIKE ? OR ACCOUNT_NAME LIKE ? OR ROLE LIKE ? OR ACTION LIKE ? OR TIMESTAMP LIKE ?)";
...
if ($yearFilter && $monthFilter) {
    $stmt->bind_param("ssssssiii", $search_term, $search_term, $search_term, $search_term, $search_term, $yearFilter, $monthFilter, $records_per_page, $offset);
} elseif ($yearFilter) {
    $stmt->bind_param("ssssssii", $search_term, $search_term, $search_term, $search_term, $search_term, $yearFilter, $records_per_page, $offset);
} elseif ($monthFilter) {
    $stmt->bind_param("ssssssii", $search_term, $search_term, $search_term, $search_term, $search_term, $monthFilter, $records_per_page, $offset);
} else {
    $stmt->bind_param("sssssii", $search_term, $search_term, $search_term, $search_term, $search_term, $records_per_page, $offset);
}
...
$total_sql = "SELECT COUNT(*) as total FROM reports WHERE (ID LIKE ? OR ACCOUNT_NAME LIKE ? OR ROLE LIKE ? OR ACTION LIKE ? OR TIMESTAMP LIKE ?)";
...
<div class="home-container">
    ...
    <div class="search-filter-container">
        ...
        <div class="search-bar">
            <form action="C3_Reports.php" method="GET">
...
<div class="button-container">
    <div class="prev-next-buttons">
        ...
        if ($yearFilter) {
            $params[] = "yearFilter=" . urlencode($yearFilter);
        }
        if ($monthFilter) {
            $params[] = "monthFilter=" . urlencode($monthFilter);
        }
        $param_string = implode("&", $params);

        if ($current_page > 1): ?>
            <a href="<?php echo $base_url . $param_string . "&page=" . ($current_page - 1); ?>" class="prev-next-button"> < Prev </a>
        <?php endif; ?>
        <?php if ($current_page < $total_pages): ?>
            <a href="<?php echo $base_url . $param_string . "&page=" . ($current_page + 1); ?>" class="prev-next-button">Next > </a>
        <?php endif; ?>
    </div>
</div>
...
<script>
    ...
    function clearSearchAndReload() {
        ...
        window.location.href = "C3_Reports.php";
    }
</script>
