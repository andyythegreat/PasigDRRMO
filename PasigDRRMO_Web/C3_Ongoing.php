...
<head>
    ...
    <title> PCDRRMO | C3 Ongoing </title>
    <link rel="stylesheet" href="C3_Ongoing_55.css">
...
if(isset($_GET['search']) && !empty($_GET['search'])) {
    $search = $_GET['search'];
    $search_query = " AND (Caller LIKE '%$search%' OR Location LIKE '%$search%' OR Involve LIKE '%$search%' OR Status LIKE '%$search%')";
}

$sql = "SELECT ID, Date, Caller, Location, Involve, Status FROM c3_locate WHERE Status != 'Resolved' AND DATE(Date) = '$currentDate' $search_query ORDER BY ID DESC";
$result = $conn->query($sql);
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $status = $row["Status"];
        $colorQuery = "SELECT Color FROM c3_status WHERE Name = ?";
        $colorStmt = $conn->prepare($colorQuery);
        $colorStmt->bind_param("s", $status);
        $colorStmt->execute();
        $colorResult = $colorStmt->get_result();
        $colorRow = $colorResult->fetch_assoc();
        $statusColor = $colorRow ? $colorRow['Color'] : '#90ee90'; 
        $colorStmt->close();
        ...
        echo "<a href='C3_Status.php?id=" . $row["ID"] . "' class='view-button'>View</a>";

        echo "<form method='post' action='C3_Completed.php'>";
        echo "<input type='hidden' name='id' value='" . $row['ID'] . "'>";
        echo "<button type='submit' class='complete-button'>Resolved</button>";
        echo "</form>";
        echo "</div>";
        ...
        echo "<div id='map-" . $row['ID'] . "' style='height: 350px; width: 100%;'></div>";
        echo "</div>";
        echo "</div>";
...
echo "<script>
    function initMap" . $row['ID'] . "() {
        ...
        var address = '" . addslashes($row["Location"]) . "';
        ...
        geocoder.geocode({ 'address': address }, function(results, status) {
            if (status === 'OK') {
                ...
                // Primary marker at the geocoded location
                ...
                // Object to store existing firetruck markers by their Username
                var firetruckMarkers = {};

                // Function to fetch and update firetruck locations
                function fetchFiretruckLocations() {
                    fetch('getTruckMarkers.php?ongoingID=" . $row['ID'] . "')
                        .then(response => response.json())
                        .then(data => {
                            // Loop through the fetched data and update the truck markers
                            data.forEach(function(location) {
                                var title = location.RespondersBarangay + ': ' + location.Username;
                                
                                // Check if a marker for this truck already exists
                                if (firetruckMarkers[location.Username]) {
                                    // Update the position of the existing marker
                                    firetruckMarkers[location.Username].setPosition(new google.maps.LatLng(location.lat, location.lng));
                                } else {
                                    ...
                                    firetruckMarkers[location.Username] = marker;
                                }
                            });

                            // Remove markers that are no longer in the updated data
                            for (var username in firetruckMarkers) {
                                var markerExists = data.some(function(location) {
                                    return location.Username === username;
                                });

                                if (!markerExists) {
                                    // Remove the marker from the map and delete from the object
                                    firetruckMarkers[username].setMap(null);
                                    delete firetruckMarkers[username];
                                }
                            }
                        })
                        .catch(error => console.error('Error fetching firetruck locations:', error));
                }

                // Fetch firetruck locations initially
                fetchFiretruckLocations();

                // Fetch firetruck locations every 1 second (to update regularly)
                setInterval(fetchFiretruckLocations, 1000);
            } else {
...
<script>
    function clearSearch() {
        ...
        window.location.href = 'C3_Ongoing.php'; // Reload the page to clear the search results
    }
</script>
...
#mobileReportDetails p {
    line-height: 2; 
    margin: 0; 
}

#mobileReportDetails strong {
    color: #333; 
}
