...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($photoPath)) {
        $checkSql = "SELECT COUNT(*) AS count FROM c3_logo";
        $result = $conn->query($checkSql);
        $row = $result->fetch_assoc();
        $logoExists = $row['count'] > 0;

        if ($logoExists) {
            $sql = "UPDATE c3_logo SET Logo = ? LIMIT 1";
        } else {
            $sql = "INSERT INTO c3_logo (Logo) VALUES (?)";
        }
        ...
        if ($stmt->execute()) {
            echo "<script>alert('Photo uploaded successfully!'); window.location.href='C3_Logo.php';</script>";
        } else {
...
?>
    ...
    <head>
        ...
        <link rel="stylesheet" href="C3_Logo35.css">
    <link rel="shortcut icon" type="image/png" href="<?php echo $logoPath; ?>">
    </head>
    <body>
    ...
    <nav class="navbar">
        <div class="logo_item">
           <img src="<?php echo $logoPath; ?>" alt="Pasig City DRRMO Logo">Pasig City DRRMO
...
<!-- Content Sections For Logo -->
<div class="add-status-container">
    <h1> Change Logo </h1>
    <form method="POST" action="C3_Logo.php" enctype="multipart/form-data">
        <label for="logo"> Select Photo:</label>
        <input type="file" id="logo" name="logo" class="status-name-input" accept="image/*" onchange="previewLogo(event)" required>
                ...
                <tr>
                    <td>
                        <div class="logo-preview-container">
                            <div id="logo-preview-container" class="logo-preview">
                                <img id="logo-preview" src="<?php echo $showLogo ? $logoPath : ''; ?>" alt="Logo Preview" style="<?php echo $showLogo ? '' : 'display: none;'; ?>">
                            </div>
                        </div>
                    </td>
                </tr>
