...
<head>
    ...
    <title> PCDRRMO | C3 Archived Records </title>
    <link rel="stylesheet" href="C3_ArchivedRecords55.css">
...
<section class="home-section">
    <h1>FIRE ALARM ARCHIVED RECORDS</h1>
    
    <div class="home-container1">
    <div class="x-account">
        <a href="C3_Records.php" class="x-link">X</a>
    </div>
    </div> 
...
<tbody>
    ...
    $fetchQuery = "SELECT * FROM c3_archiverecords WHERE ID = ?";
    if ($stmt = $conn->prepare($fetchQuery)) {
...
$insertQuery = "INSERT INTO c3_incidentreport (IncidentType, IncidentNumber, DateTime, Location, Barangay, Caller, Date, Time, Dispatcher, Remarks) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    $insertStmt->bind_param("ssssssssss", 
        ...
        $row['Remarks']      
                ...
                if ($insertStmt->execute()) {
                    // Delete from c3_archiverecords
                    $deleteQuery = "DELETE FROM c3_archiverecords WHERE ID = ?";
                    if ($deleteStmt = $conn->prepare($deleteQuery)) {
                        $deleteStmt->bind_param("i", $restoreId);
                        $deleteStmt->execute();
                        $deleteStmt->close();
                    }
                    echo "<script>alert('Record restored successfully.'); window.location.href='C3_ArchivedRecords.php';</script>";
                } else {
                    echo "<script>alert('Error restoring record: " . $insertStmt->error . "');</script>";
                }
                $insertStmt->close();
            }
        } else {
            echo "<script>alert('No record found to restore.');</script>";
        }
        $stmt->close();
    }
    $conn->close();
                ...
                // Fetch records from c3_archiverecords
                $query = "SELECT ID, IncidentType, IncidentNumber, DateTime, Location, Barangay, Caller, Date, Time, Dispatcher, Remarks FROM c3_archiverecords";
                        ...
                        echo "<td>
                                <div class='btn-container'>
                                    <button class='view-button'>
                                        <a href='C3_ArchivedRecords.php?restore_id=$id' class='restore'> 
