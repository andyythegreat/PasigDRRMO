    ...
    if ($stmt->execute()) {
        ...
        if ($stmt_accept->execute()) {
            ...
            if ($deleteStmt->execute()) {
                echo json_encode(['message' => 'Alert accepted successfully']);
            } else {
                echo json_encode(['message' => 'Alert accepted, but failed to delete from brgy_locate.']);
            }

            $deleteStmt->close();
        } else {
            echo json_encode(['message' => 'Alert accepted, but failed to insert into brgy_accept.']);
        }

        $stmt_accept->close();
    } else {
        echo json_encode(['message' => 'Error accepting alert.']);
    }
